...
def summary_num_var(df, column, gbe=False, sbs=False):
    ...
    print('--'*20)
    kolmogorov_norm_test(df[column])

def print_skewness_kurtosis(df, column): 
    print(f"Skewness of {column}: {stats.skew(df[column])}")
    print(f"Kurtosis of {column}: {stats.kurtosis(df[column])}")

def kolmogorov_norm_test(data):
