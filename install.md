# Churn Predictive Dashboard guide installation
...
conda activate churn_predictive
```
...
mamba activate churn_predictive
```
or 
...
To move beyond notebook prototyping, all reusable code should go into the `churn_predictor/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `churn_predictor` folder and use the modules inside your notebooks :
...
```
This will install the project module in "editable" mode so that any changes you make to the code in the churn_predictor/ folder will be reflected immediately without needing to reinstall.
...
```python
from churn_predictor.utils.paths import data_dir
data_dir()
```
...
```python
from churn_predictor.utils.paths import data_raw_dir
